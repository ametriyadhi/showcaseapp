...
export function LoginForm({ appName, appLogo, appDescription }: { appName: string, appLogo: string, appDescription: string }) {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-3xl p-8 border border-slate-200 shadow-xl shadow-slate-200/50">
        <div className="text-center mb-8">
          {appLogo ? (
            <img src={appLogo} alt={appName} className="w-16 h-16 mx-auto mb-4 object-contain" />
          ) : (
            <div className="w-16 h-16 bg-blue-600 rounded-2xl mx-auto flex items-center justify-center mb-4 shadow-lg shadow-blue-500/30 text-white text-2xl font-black">
              {appName ? appName.substring(0, 3).toUpperCase() : "AIS"}
            </div>
          )}
          <h1 className="text-2xl font-black text-slate-900">{appName}</h1>
          <p className="text-slate-500 mt-2 text-sm">{appDescription}</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <input
              ...
              placeholder="admin@apphub.local"
              required
            />
          </div>
          <div>
